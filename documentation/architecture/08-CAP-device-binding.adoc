...
For Arqua42, we recommend **Strategy 2 (Token Check-out)** combined with an **Admin Override**. This aligns with the "Stammdaten" workflow where judges are assigned to competitions beforehand. The "Check-out" happens when the device creates the offline cache.
